...
namespace NinjaScan_GUI.AHRS
{
    ...
    public class MadgwickAHRS
    {
        ...
        public float[] Euler_deg { get; set; }

        public void Quaternion2Euler(float[] q)
        {
            float roll = Euler[0];
            float pitch = Euler[1];
            float yaw = Euler[2];
            roll = (float)Math.Atan2((double)(2 * q[2] * q[3] - q[0] * q[1]),
                (double)(2 * q[0] * q[0] - 1 + 2 * q[3] * q[3]));
            pitch = - (float)Math.Atan2((double)(2 * q[1] * q[3] + q[0] * q[2]),
                (double)Math.Sqrt(1 - Math.Pow((double)(2 * q[1] * q[3] + 2 * q[0] * q[2]), 2)));
            yaw = (float)Math.Atan2((double)(2 * q[1] * q[2] - q[0] * q[3]),
                (double)(2 * q[0] * q[0] - 1 + 2 * q[1] * q[1]));
            Euler[0] = roll;
            Euler[1] = pitch;
            Euler[2] = yaw;
            Euler_deg[0] = Rad2Deg(Euler[0]);
            Euler_deg[1] = Rad2Deg(Euler[1]);
            Euler_deg[2] = Rad2Deg(Euler[2]);
        }
        ...
        public MadgwickAHRS(float samplePeriod, float beta)
        {
            ...
            Euler = new float[] { 0f, 0f, 0f };
            Euler_deg = new float[] { 0f, 0f, 0f };
        }
